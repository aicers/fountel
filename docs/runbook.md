...
The sops-encrypted dev secrets ship **already committed**: the dotenv bundle
`deploy/dev/secrets.enc.env` and the instance GnuPG key
`deploy/dev/gnupg.secret.enc.asc`, both keyed to the dev recipient in
`.sops.yaml`. You do **not** generate them — you only need a key that can
decrypt them. If you are a new operator, get added as a recipient first (see
[secrets.md](secrets.md), "Additional operator"); then `up.sh` decrypts for
you in step 2.

Verify both committed files are real ciphertext (no plaintext leaked):

```sh
grep -q 'sops' secrets.enc.env && echo "OK: sops-encrypted"
grep -Eq 'ENC\[' secrets.enc.env && echo "OK: values are encrypted"
grep -q 'sops' gnupg.secret.enc.asc && echo "OK: GnuPG key sops-encrypted"
grep -q 'BEGIN PGP PRIVATE KEY' gnupg.secret.enc.asc \
  && echo "LEAK: private key in cleartext!" || echo "OK: no cleartext private key"
```

> Standing up a brand-new environment, or rotating? `./bin/secrets-init.sh`
> generates the key, the secret values, and the instance GnuPG key, writing
> both encrypted files (`--rotate` regenerates them); it additionally needs
> `gnupg` on PATH. For the committed dev environment this is **not** part of
> normal bring-up.
...
`secrets/misp.secrets.env` and the instance GnuPG key into
`secrets/misp.gnupg.asc`, seeds that key into the `misp_gnupg` volume before
boot (so misp-core adopts the committed signing key instead of autogenerating
one), runs `docker compose up -d`, and waits for `misp-core` to report healthy.
First boot pulls images and initializes the DB, so it can take a few minutes.
...
### Instance GnuPG key (seeded from the committed key)
...
This prints the instance signing-key fingerprint. Because the key is committed
sops material (not autogenerated), this fingerprint is **stable** across a full
reset — `docker compose down -v` then `./bin/up.sh` reproduces the same key. An
autogenerated key would change on every reset.
